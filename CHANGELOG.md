...
* **testing:** `AdapterSuite.run` accepts `mixed_sort` (default `:sql`). `:sql` keeps SQLite and MemoryAdapter mixed `metadata` order (`[2, "10", [1]]`). `:bson` is MongoDB native order (`[[1], 2, "10"]`). Backend specs run the `:bson` branch with a spec-only MemoryAdapter subclass (production MemoryAdapter stays SQLite-like).
